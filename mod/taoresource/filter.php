 ...
 */
    ...
    function taoresource_filter($courseid, $text) {
    ...
    /// Create a list of all the taoresources to search for.  It may be cached already.
        if (empty($resourcelist)) {
             ...
             */
            $resource_sql  = "SELECT cm.id, r.name 
                FROM {$CFG->prefix}taoresource r, 
                     {$CFG->prefix}course_modules cm, 
                     {$CFG->prefix}modules m
                WHERE m.name = 'taoresource' AND
                        cm.module = m.id AND
                        cm.visible =  1 AND
                ...
                if (!empty($strippedname)) {
                    $resourcelist[] = new filterobject($currentname,
                            '<a class="resource autolink" title="'.$strippedname.'" href="'.
                             $CFG->wwwroot.'/mod/taoresource/view.php?id='.$resource->id.'" '.$CFG->frametarget.'>', 
                             '</a>', false, true);
                }
            }
