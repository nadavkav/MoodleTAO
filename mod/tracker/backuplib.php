<?php //$Id: backuplib.php,v 1.2.2.4 2008/10/07 20:06:23 diml Exp $
    ...
    function tracker_issueattribute_ids_by_course ($course) {
        ...
        $sql = "
            SELECT 
                ...
                ia.trackerid
            FROM 
                {$CFG->prefix}tracker_issueattribute AS ia, 
                {$CFG->prefix}tracker AS t
            WHERE 
                t.course = '{$course}' AND
                ia.trackerid = t.id
    ...
    function tracker_issuecomment_ids_by_course ($course) {
        ...
        $sql = "
            SELECT 
                ...
                ic.trackerid
            FROM 
                {$CFG->prefix}tracker_issuecomment AS ic, 
                {$CFG->prefix}tracker AS t
            WHERE 
                t.course = '{$course}' AND
                ic.trackerid = t.id
    ...
    function tracker_preferences_ids_by_course ($course) {
        ...
        $sql = "
            SELECT 
                ...
                p.trackerid
            FROM 
                {$CFG->prefix}tracker_preferences AS p, 
                {$CFG->prefix}tracker AS t
            WHERE 
                t.course = '{$course}' AND
                p.trackerid = t.id
