<?PHP  // $Id: rpclib.php,v 1.3.2.3 2008/10/17 07:25:51 diml Exp $
...
/*
* sends tracker information to remote caller.
* @param int $trackerid
...
* the remote user has capability to post in. Note that this
...
function tracker_rpc_get_instances($username, $remotehostroot){
    ...
    if(!empty($trackers)){
        foreach($trackers as $id => $tracker){
            ...
            $modulecontext = get_context_instance(CONTEXT_MODULE, $id);
...
* @param int $userid the local userid the post should come from
* @param string $remote_issue a JSON encoded variable containing all
* information about an issue.
* @return the local issue record id
*/
function tracker_rpc_post_issue($username, $remotehostroot, $trackerid, $remote_issue){
    ...
    if (! $followid = insert_record('tracker_issue', $issue)){
        // TODO : error report
        $response->status = RPC_FAILURE;
    ...
    if (!empty($issue->attributes)){
        $used = tracker_getelementsused_by_name($trackerid);
    ...
    if (!insert_record('tracker_issuecomment', $issuecomment)){
        $response->status = RPC_FAILURE;
