<?php //$Id$
     ...
     *   messages
     */
    function import_backup_file_silently($pathtofile,$destinationcourse,$emptyfirst=false,$userdata=false, $preferences=array(), $restoreto=null) {
        ...
        if (!file_exists($pathtofile)) {
            mtrace($debuginfo.'File '.$pathtofile.' could not be found');
            return false;
        }

        if (!is_readable($pathtofile)) {
            mtrace($debuginfo.'File '.$pathtofile.' wasn\'t readable');
            return false;
        }
        ...
        $restore->preferences = $preferences;
	...
	// create exception for learning path creation.  not especially happy with this method but restricted by core code.
        $restore->course_format = $restore->course_format = (isset($preferences['course_format']) ? $preferences['course_format'] : 0);
