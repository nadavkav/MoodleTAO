...
function useredit_shared_definition(&$mform) {
    ...
    //lock firstname and lastname if user_info_field is set - (used by the certificate module)
    if (record_exists('tao_user_certification_status', 'userid', $user->id, 'status', 'approved')) {
        $systemcontext = get_context_instance(CONTEXT_SYSTEM);
        if(!has_capability('moodle/user:update', $systemcontext)) {
            $mform->hardFreeze('firstname');
            $mform->setConstant('firstname', $user->firstname);
            $mform->hardFreeze('lastname');
            $mform->setConstant('lastname', $user->lastname);
            $mform->addElement('static', 'lockedwarning', '',notify(get_string('namelockedwarning','local'),'notifyproblem','center',true));
        } else {
            $mform->addElement('static', 'lockedwarning', '',notify(get_string('namelockedwarningadmin','local'),'notifyproblem','center',true));
        }
    }
