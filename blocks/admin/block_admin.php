...
class block_admin extends block_list {
    ...
    /// Course editing on/off
        ...
        } elseif (has_capability('moodle/local:canchangelpsettings',$context)) { //show basic settings page if this user has the capability
            $this->content->items[]='<a href="'.$CFG->wwwroot.'/local/lp/editlearningpath.php?id='.$this->instance->pageid.'">'.get_string('settings').'</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/settings.gif" class="icon" alt="" />';
    ...
    /// Unenrol link
        if (empty($course->metacourse) && ($course->id!==SITEID)) {
            if (has_capability('moodle/legacy:guest', $context, NULL, false)) {   // Are a guest now
             /* TAO: don't show enrol link if logged in as guest!
