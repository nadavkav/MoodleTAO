...
function fm_print_share_course_members($id, $linkid=NULL, $type=0, $groupid=0, $allpersons, $paginated) {
    global $CFG, $USER;
	...
	$strcbx = "<input type=\"checkbox\" name=\"cbu[]\" value=\"0\" onClick=\"selectboxes(this, 1)\">&nbsp;";
    if ($paginated) {
        $strcbx .= get_string("selectallonthispage", 'block_file_manager');
    } else {
        $strcbx .= get_string("selectall", 'block_file_manager');
    }
	$strlastname = "<a href=\"$CFG->wwwroot/$fmdir/sharing.php?id=$id&groupid=$groupid&linkid=$linkid&tsort=sortlast\">".get_string("userlastname",'block_file_manager')."</a>";
	$strfirstname = "<a href=\"$CFG->wwwroot/$fmdir/sharing.php?id=$id&groupid=$groupid&linkid=$linkid&tsort=sortfirst\">".get_string("userfirstname",'block_file_manager')."</a>";

	$table->head = array($strcbx, $strlastname, $strfirstname);

    
    $allusers = array();
	if (empty($allpersons)) { // If there is nobody in this course, then print the message "no users in this course"
	...
	if (!empty($allusers)) {
		...
		foreach ($allusers as $au) {
			if ($au->userid != $USER->id) {		// Cant share to yourself
				...
				$table->data[] = array($cbx, format_text($au->lastname,FORMAT_PLAIN), format_text($au->firstname,FORMAT_PLAIN));
			}
		}
	} 
