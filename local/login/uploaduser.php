<?php  // $Id$
/*  CUSTOM version of admin/uploaduser.php for TAO
 *  doesn't use Adminsettings to display page
 *  sets users as confirmed =0
 *  triggers local functions to send e-mail and to assign roles 
*/
...
require_capability('moodle/local:bulkinvitenewuser', $sitecontext);
...
if ($formdata = $mform->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($existinguser) {
            ...
            $user->confirmed = 0; //TAO: Participant Registration hack
            ...
            if ($user->id = insert_record('user', addslashes_recursive($user))) {
                ...
                //now do role assignment for PT
                if (function_exists('local_user_signup') ) {
                    local_user_signup($user);
                }
                //now do e-mail conf to user.
                if (function_exists('local_send_confirmation_email') ) {
                    if(! local_send_confirmation_email($user)) {
...
function uploaduser_print_header() {
    print_header(get_string('userbulk','admin'), get_string('userbulk','admin'), array(), "");
}
function uploaduser_print_footer() {
    print_footer();
}
?>