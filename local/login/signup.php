<?php  // $Id$
    ...
    if ($mform_signup->is_cancelled()) {
        ...
        $user->username    = 'invite_'.random_string(15);
        if (empty($user->password)) {
            $user->password = random_string(25);
        }
        if ($authplugin->user_signup($user, false)) {; // prints notice and link to login/index.php
            $emailconfirm = get_string('inviteauser','local');
            $navlinks = array();
            $navlinks[] = array('name' => $emailconfirm, 'link' => null, 'type' => 'misc');
            $navigation = build_navigation($navlinks);

            print_header($emailconfirm, $emailconfirm, $navigation);
            notice(get_string('emailconfirmsent', 'local', $user->email), "$CFG->wwwroot/index.php");
            exit; //never reached
        }
    }
