...
function add_menu_listeners(e) {
    ...
    var submitbutton = YAHOO.util.Dom.get('taoviewratingsubmit');
    ...
    var menus = YAHOO.util.Dom.getElementsByClassName('taoviewratingmenu', 'select');
...
function perform_rate(e, menu) {
    ...
    var url = rate_ajax_config.wwwroot +  '/local/mahara/rate_ajax.php?artefactid=' + menu.name + '&rate=' + menu.value + '&sesskey=' + rate_ajax_config.sesskey;
...
function display_response(menu, response) {
    ...
    var ratingsSpan = YAHOO.util.Dom.getFirstChildBy(ratingsDiv, function(el){return YAHOO.util.Dom.hasClass(el,'taoviewratingtext');});

    /** span doesn't exist (first rate), add it, shouldn't happen ever but... fallback**/
    if (!ratingsSpan) {
        ratingsSpan = document.createElement('span');
        YAHOO.util.Dom.addClass(ratingsSpan, 'taoviewratingtext');
