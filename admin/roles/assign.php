...
/// These are needed early because of tabs.php
    ...
    tao_role_filter($assignableroles, $nameswithcounts, $context);
...
/// Process any incoming role assignments before printing the header.
                    ...
                    if ($inmeta) {
                        if (has_capability('moodle/course:managemetacourse', $context, $adduser->id)) {
                            ...
                            if (!empty($managerroles) and !array_key_exists($roleid, $managerroles)) {
                                $erruser = get_record('user', 'id', $adduser->id, 'id, firstname, lastname');
    ...
    /// Process any local commands
        if(function_exists('local_role_processing')) {
...
function tao_role_filter(&$assignableroles, &$nameswithcounts, $context) {
        //TAOEDIT: hide standard roles if frontpage roles
        $isfrontpage = $context->contextlevel == CONTEXT_COURSE && $context->instanceid == SITEID;
        if ($isfrontpage) {
            $roleflip = array_flip($assignableroles);
            if (isset($roleflip['Teacher'])) {
                unset($assignableroles[$roleflip['Teacher']]);
                unset($nameswithcounts[$roleflip['Teacher']]);
            }
            if (isset($roleflip['Non-editing teacher'])) {
                unset($assignableroles[$roleflip['Non-editing teacher']]);
                unset($nameswithcounts[$roleflip['Non-editing teacher']]);
            }
            if (isset($roleflip['Student'])) {
                unset($assignableroles[$roleflip['Student']]);
                unset($nameswithcounts[$roleflip['Student']]);
            }
        }
}
?>