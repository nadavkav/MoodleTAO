...
define('FRONTPAGECMS',5); //added for CMS course type
...
/// Next, have there been any modifications to the course structure?
    ...
    if ($logs) {
        ...
        foreach ($logs as $key => $log) {
            ...
            if (preg_match("/label$/", $info[0])) {     // Labels are ignored in recent activity - Modified to include customlabels
                continue;
            }
        ...
        foreach ($sectionmods as $modnumber) {
            ...
            if ($mod->modname == "label") {
                ...
                if (!empty($CFG->enablegroupings) && !empty($mod->groupingid) && has_capability('moodle/course:managegroups', get_context_instance(CONTEXT_COURSE, $course->id))) {
                 ...
                 echo format_text($instancename, FORMAT_HTML);
                 if (!$mod->visible) {
                     echo "</span>";
                 }
...
function set_coursemodule_trackprogress($id, $trackprogress) {
    return set_field("course_modules", "trackprogress", $trackprogress, "id", $id);
}
/**
