...
function page_blocks_print_group(&$pageblocks, $position) {
    ...
    foreach($pageblocks[$position] as $instance) {
        ...
        if (($isediting || !empty($editalways)) && empty($instance->pinned)) {
        ...
        if ($obj->hide_header() && empty($obj->edit_controls) && $position=='c') {
            //force center blocks to use the header.
            $obj->edit_controls = '<div></div>';
        }
 ...
 * @return void
 **/
function page_format_execute_url_action($action = NULL, $course = NULL) {
    ...
    if (!empty($action)) {
        ...
        switch ($action) {
            ...
            default:
                ...
                if (file_exists($file)) {
                    print '<div id="page-format-action">';
                    include($file);
                    print '</div>';
