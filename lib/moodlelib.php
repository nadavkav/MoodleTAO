 ...
 * @param bool $enrolredirect Set to false to handle non-enrolment in a non-standard way, function will return "true".
 */
function require_login($courseorid=0, $autologinguest=true, $cm=null, $setwantsurltome=true, $enrolredirect=true) {
...
/// groupmembersonly access control
    ...
    /// For non-guests, check if they have course view access
        ...
        if ($enrolredirect) {
            //so see if they want to enrol - default action
            $SESSION->wantsurl = $FULLME;
            redirect($CFG->wwwroot .'/course/enrol.php?id='. $COURSE->id);
            die;
        } 

        // return true (because the user "requires login" I guess) and let calling function handle the response
        return true;
 ...
 * @uses $USER
 */
function require_logout($log=true) {
...
function create_guest_record() {
    ...
    $guest->mnethostid  = isset($CFG->mnet_localhost_id) ? $CFG->mnet_localhost_id : 0;
    $guest->confirmed   = 1;
    $guest->lang        = $CFG->lang;
