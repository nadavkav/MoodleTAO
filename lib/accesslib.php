    ...
    $sql = "SELECT 1
             ...
             WHERE ctx.contextlevel = 10
                   ...
                   AND rc.capability IN
                    ('moodle/site:config', 'moodle/legacy:admin', 'moodle/site:doanything')
          GROUP BY rc.capability
            HAVING SUM(rc.permission) > 0";
    ...
    if (empty($userid)) {
        if ($context->contextlevel == CONTEXT_COURSE) {
            ...
            if (!$cm = get_record('course_modules', 'id', $context->instanceid)) {
                print_error('Incorrect module'); // error >> print_error (nadavkav)
            }
            if (!$course = get_record('course', 'id', $cm->course)) {
                print_error('Incorrect course.'); // error >> print_error (nadavkav)
            }
...
// cleanup all references to this role, ignore errors
    if ($success) {
        ...
        delete_records('role_context_levels', 'roleid', $roleid);
    }
...
function get_cached_capabilities($component='moodle') {
   ...
   } else { //downbrowses the cascade chain of local{n} subdirs. gets local, locallocal etc...
       if (preg_match('/^local/', $component)) {
           $storedcaps = array();
           $pat = 'local';
           $path = '/local';
            while (is_dir("{$CFG->dirroot}{$path}")){
               if ($tmp = get_records_select('capabilities',
                           "name LIKE 'moodle/{$pat}:%'")) {
                   $storedcaps = array_merge($storedcaps, $tmp);
               }
               $pat .= 'local';
               $path .= '/local';
            }
...
function print_context_name($context, $withprefix = true, $short = false) {
    ...
    switch ($context->contextlevel) {
        ...
        case CONTEXT_MODULE: // 1 to 1 to course
            if ($cm = get_record_sql('SELECT cm.*, md.name AS modname FROM {course_modules} cm ' .
                    'JOIN {modules} md ON md.id = cm.module WHERE cm.id = ' . $context->instanceid)) {
                if ($mod = get_record($cm->modname, 'id',  $cm->instance)) {
                        if ($withprefix){
...
function get_context_url($context) {
    ...
    switch ($context->contextlevel) {
        ...
        case CONTEXT_MODULE: // 1 to 1 to course
            if ($modname = get_field_sql('SELECT md.name AS modname FROM {course_modules} cm ' .
                    'JOIN {modules} md ON md.id = cm.module WHERE cm.id = ' . $context->instanceid)) {
 ...
 */
function get_sorted_contexts($select) {
    if ($select) {
        $select = 'WHERE ' . $select;
    }
...
function get_assignable_roles($context, $rolenamedisplay = ROLENAME_ALIAS, $withusercounts = false) {
    ...
    if ($withusercounts) {
        $extrafields = ', (SELECT count(u.id)
                ...
                WHERE cra.roleid = ro.id AND cra.contextid = ' . $context->id . ' AND u.deleted = 0
                ) AS usercount';
    }

    if (!$roles = get_records_sql("
             SELECT ro.id, ro.name$extrafields
              ...
              WHERE rcl.contextlevel = $context->contextlevel
            ...
            SELECT ro.*
              ...
              JOIN (SELECT DISTINCT r.id
                     ...
                     WHERE ra.userid = $USER->id AND ra.contextid IN ($contexts)
                      ...
                      AND NOT EXISTS (SELECT 1 FROM {role_capabilities} irc
...
function get_overridable_roles($context, $rolenamedisplay = ROLENAME_ALIAS, $withcounts = false) {
    ...
    if ($withcounts) {
        $extrafields = ', (SELECT count(rc.id) FROM {role_capabilities} rc
                WHERE rc.roleid = ro.id AND rc.contextid = ' . $context->id . ') AS overridecount';
    }
...
function get_role_contextlevels($roleid) {
    return get_records_menu('role_context_levels', 'roleid',   $roleid,
...
function set_role_contextlevels($roleid, array $contextlevels) {
    if (!delete_records('role_context_levels', 'roleid', $roleid)) {
        print_error('couldnotdeleterolecontextlevels', '', '', $roleid);
    }
    $rcl = new stdClass;
    $rcl->roleid = $roleid;
    foreach ($contextlevels as $level) {
        $rcl->contextlevel = $level;
        if (!insert_record('role_context_levels', $rcl, false, true)) {
            print_error('couldnotdeleterolecontextlevels', '', '', $rcl);
        }
    }
}
...
function get_role_users($roleid, $context, $parent=false, $fields='',
        ...
        $limitfrom='', $limitnum='', $extrawheretest='') {
...
function switch_roles($first, $second) {
    $temp = get_field('role', 'MAX(sortorder) + 1');
    $result = set_field('role', 'sortorder', $temp, 'sortorder', $first->sortorder);
    $result = $result && set_field('role', 'sortorder', $first->sortorder, 'sortorder', $second->sortorder);
    $result = $result && set_field('role', 'sortorder', $second->sortorder, 'sortorder', $temp);
    return $result;
}
