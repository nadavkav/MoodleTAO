 ...
 *
 */
function get_courses($categoryid="all", $sort="c.sortorder ASC", $fields="c.*", $courseids=null) {
    ...
    if (is_array($courseids)) {
        ...
        $idselect .= ' c.id IN (' . implode(', ', $courseids) . ')';
    } else {
        $idselect = '';
    }
...
function get_coursemodule_from_id($modulename, $cmid, $courseid=0) {
    ...
    global $CFG;
    if (empty($modulename)) {
        if (!$modulename = get_field_sql("SELECT md.name
                                                 FROM {modules} md
